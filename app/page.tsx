...
async function saveDialer(data:any){
 ...
 return <div className="app">
  ...
  <main className="content">
   ...
   {mode==="config"&&<Settings operator={operator} users={users} onSaveUser={saveUserConfig} channelConfig={channelConfig} onSaveChannels={saveChannels} dialerConfig={dialerConfig} onSaveDialer={saveDialer}/>}
...
function CRM({leads,stages,onMove,onOpen}:{leads:Lead[];stages:Stage[];onMove:(id:string,s:string)=>void;onOpen:(l:Lead)=>void}){
 const operational=[["numero_invalido","Número inválido"],["sem_perfil","Sem perfil"],["nao_atendeu","Não atendeu"],["nao_interessado","Não interessado"],["finalizado","Finalizado"]];
 return <div className="stack"><div className="panel"><PanelTitle title="CRM / Esteira de vendas" subtitle="Leads por etapa e resultados operacionais."/><div className="kanban">{stages.map(s=><div className="kanbanCol" key={s.id}><div className="kanbanHead"><span style={{background:s.cor}}/>{s.nome}<b>{leads.filter(l=>l.status===s.nome.toLowerCase()).length}</b></div>{leads.filter(l=>l.status===s.nome.toLowerCase()).slice(0,30).map(l=><button className="leadCard" key={l.id} onClick={()=>onOpen(l)}><div className="miniAvatar">{initials(l.nome)}</div><div><b>{l.nome}</b><small>{l.produto||"Consignado"} • {l.cidade||"—"}</small></div><span>›</span></button>)}{!leads.some(l=>l.status===s.nome.toLowerCase())&&<div className="emptyCol">Sem registros</div>}</div>)}</div></div><div className="panel"><PanelTitle title="Resultados operacionais" subtitle="Aqui ficam Número inválido, Sem perfil, Não atendeu e outros resultados que não entram no funil."/><div className="resultCards">{operational.map(([id,label])=>{const arr=leads.filter(l=>l.status===id);return <div className="resultCard" key={id}><div><b>{label}</b><small>{arr.length} lead(s)</small></div><div className="resultList">{arr.slice(0,8).map(l=><button key={l.id} onClick={()=>onOpen(l)}>{l.nome}<span>›</span></button>)}{arr.length>8&&<small>+ {arr.length-8} outros</small>}</div></div>})}</div></div></div>
}
...
function Campaigns({rows,onCreate,onToggle,onDelete}:{rows:Campaign[];onCreate:(d:{nome:string;produto:string;inicio_at?:string;fim_at?:string})=>void;onToggle:(id:string,status:string)=>void;onDelete:(id:string)=>void}){
 ...
 const submit=()=>{if(!nome.trim())return;onCreate({nome,produto,inicio_at:inicio?new Date(inicio).toISOString():undefined,fim_at:fim?new Date(fim).toISOString():undefined});setNome("");setProduto("Consignado");setInicio("");setFim("");setOpen(false)};
 return <div className="panel"><div className="toolbar"><PanelTitle title="Campanhas" subtitle="Crie, programe, ative, pause e remova campanhas."/><button className="btn primary" onClick={()=>setOpen(true)}>＋ Nova campanha</button></div><div className="campaignGrid">{rows.map(c=><div className="campaignCard" key={c.id}><div className="campaignIcon">▣</div><div className="campaignMain"><b>{c.nome}</b><small>{c.produto||"Sem produto"} • {c.status}</small><small>{c.inicio_at?"Início: "+new Date(c.inicio_at).toLocaleString("pt-BR"):"Sem início"} {c.fim_at?" → "+new Date(c.fim_at).toLocaleString("pt-BR"):""}</small></div><div className="campaignActions"><span className={"pill "+(c.status==="ativa"?"":"warning")}>{c.status}</span><button className="tableBtn" onClick={()=>onToggle(c.id,c.status)}>{c.status==="ativa"?"Pausar":"Ativar"}</button><button className="tableBtn dangerText" onClick={()=>onDelete(c.id)}>Remover</button></div></div>)}{!rows.length&&<Empty title="Nenhuma campanha" text="Crie a primeira campanha para organizar a operação."/>}</div>{open&&<div className="modal" onClick={()=>setOpen(false)}><div className="modalBox smallModal" onClick={e=>e.stopPropagation()}><div className="toolbar"><div><div className="eyebrow">NOVA CAMPANHA</div><h2>Programar campanha</h2><p>Defina quando ela deve ficar ativa.</p></div><button className="btn" onClick={()=>setOpen(false)}>Fechar</button></div><div className="field"><label>Nome</label><input value={nome} onChange={e=>setNome(e.target.value)} placeholder="Ex.: INSS Setembro"/></div><div className="field"><label>Produto</label><input value={produto} onChange={e=>setProduto(e.target.value)} placeholder="Consignado"/></div><div className="twoFields"><div className="field"><label>Início</label><input type="datetime-local" value={inicio} onChange={e=>setInicio(e.target.value)}/></div><div className="field"><label>Fim</label><input type="datetime-local" value={fim} onChange={e=>setFim(e.target.value)}/></div></div><button className="btn primary full big" onClick={submit}>Salvar campanha</button></div></div>}</div>
}
...
function Reports({leads,npd,calls,returns}:{leads:Lead[];npd:any[];calls:any[];returns:ReturnRow[]}){
 const[month,setMonth]=useState(new Date().toISOString().slice(0,7));const monthCalls=calls.filter(c=>String(c.created_at||"").slice(0,7)===month),monthReturns=returns.filter(r=>r.data_hora.slice(0,7)===month);const total=monthCalls.length;const contracts=monthCalls.filter(c=>c.resultado==="Contrato").length;const opportunities=monthCalls.filter(c=>["Interessado","Simulação","Proposta","Contrato"].includes(c.resultado)).length;const first=new Date(month+"-01T00:00:00"),start=(first.getDay()+6)%7,days=new Date(first.getFullYear(),first.getMonth()+1,0).getDate();
 const download=()=>{const header=["Data","Hora","Cliente","Resultado","Observação"];const lines=monthCalls.map(c=>[new Date(c.created_at).toLocaleDateString("pt-BR"),new Date(c.created_at).toLocaleTimeString("pt-BR"),c.leads?.nome||"",c.resultado||"",c.observacao||""]);const ret=monthReturns.map(r=>[new Date(r.data_hora).toLocaleDateString("pt-BR"),new Date(r.data_hora).toLocaleTimeString("pt-BR"),r.lead?.nome||"", "Retorno",r.observacao||""]);const csv=[header,...lines,...ret].map(row=>row.map((v:any)=>'"'+String(v).replace(/"/g,'""')+'"').join(";")).join("\n");const blob=new Blob(["\ufeff"+csv],{type:"text/csv;charset=utf-8"});const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="relatorio_"+month+".csv";a.click();URL.revokeObjectURL(url)};
 ...
 return <div className="stack"><div className="panel"><div className="toolbar"><PanelTitle title="Relatório mensal" subtitle="Escolha o mês, veja o calendário e baixe os dados completos."/><div className="actionRow"><input className="monthInput" type="month" value={month} onChange={e=>setMonth(e.target.value)}/><button className="btn primary" onClick={download}>↓ Baixar CSV</button></div></div><div className="metricGrid"><Metric title="Ligações" value={total} icon="☎" hint="no mês selecionado"/><Metric title="Oportunidades" value={opportunities} icon="↗" hint="interesse, simulação, proposta ou contrato"/><Metric title="Contratos" value={contracts} icon="✓" hint="tabulados no mês"/><Metric title="Retornos" value={monthReturns.length} icon="◷" hint="agendados no mês"/><Metric title="NPD" value={npd.length} icon="⊘" hint="ativos agora"/></div></div><div className="panel"><PanelTitle title="Calendário operacional" subtitle="Dias com atividade aparecem marcados."/><div className="calendar"><div className="calendarHead">{["Seg","Ter","Qua","Qui","Sex","Sáb","Dom"].map(x=><b key={x}>{x}</b>)}</div><div className="calendarGrid">{Array.from({length:start}).map((_,i)=><span className="calendarBlank" key={"x"+i}/>)}{Array.from({length:days}).map((_,i)=>{const d=i+1,n=totalByDay(d);return <div className="calendarDay reportDay" key={d}><b>{d}</b>{n>0&&<span>{n}</span>}</div>})}</div></div></div><div className="panel"><PanelTitle title="Registros do mês" subtitle={(monthCalls.length+monthReturns.length)+" eventos encontrados."}/><div className="tableWrap"><table><thead><tr><th>Data/hora</th><th>Cliente</th><th>Tipo</th><th>Resultado</th><th>Observação</th></tr></thead><tbody>{monthCalls.map(c=><tr key={"c"+c.id}><td>{new Date(c.created_at).toLocaleString("pt-BR",{dateStyle:"short",timeStyle:"short"})}</td><td>{c.leads?.nome||"—"}</td><td>Ligação</td><td><span className="pill">{c.resultado||"—"}</span></td><td>{c.observacao||"—"}</td></tr>)}{monthReturns.map(r=><tr key={"r"+r.id}><td>{new Date(r.data_hora).toLocaleString("pt-BR",{dateStyle:"short",timeStyle:"short"})}</td><td>{r.lead?.nome||"—"}</td><td>Retorno</td><td><span className="pill warning">Agendado</span></td><td>{r.observacao||"—"}</td></tr>)}</tbody></table>{!monthCalls.length&&!monthReturns.length&&<Empty title="Sem registros no mês" text="Quando a operação gerar ligações e retornos, eles aparecerão aqui."/>}</div></div></div>
}
function Npd({rows,onRemove}:{rows:any[];onRemove:(r:any)=>void}){return <div className="panel"><PanelTitle title="Não Perturbe" subtitle="Selecione um registro para remover o bloqueio e devolver o lead à fila."/><div className="tableWrap"><table><thead><tr><th>Nome</th><th>CPF</th><th>Telefone</th><th>Origem</th><th>Motivo</th><th>Ação</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><b>{r.nome||"—"}</b></td><td>{r.cpf?mask(r.cpf):"—"}</td><td>{r.telefone||"—"}</td><td>{r.origem}</td><td>{r.motivo||"—"}</td><td><button className="tableBtn dangerText" onClick={()=>onRemove(r)}>Remover do NPD</button></td></tr>)}</tbody></table>{!rows.length&&<Empty title="Nenhum bloqueio ativo" text="Use o botão Não ligar mais na fila."/>}</div></div>}function Settings({operator,users,onSaveUser,channelConfig,onSaveChannels,dialerConfig,onSaveDialer}:{operator:any;users:UserRow[];onSaveUser:(id:string,p:Record<string,boolean>,pref:Record<string,any>,ativo:boolean,perfil:string)=>void;channelConfig:any;onSaveChannels:(d:any)=>void;dialerConfig:any;onSaveDialer:(d:any)=>void}){
 const isAdmin=operator?.perfil==="admin";const moduleList=[["dashboard","Visão geral"],["discador","Discador"],["crm","CRM"],["leads","Leads"],["campanhas","Campanhas"],["retornos","Retornos"],["telefonia","Telefonia"],["mensagens","Omnichannel"],["relatorios","Relatórios"],["npd","Não Perturbe"],["config","Configurações"]];
 const[sel,setSel]=useState<UserRow>(operator||users[0]);useEffect(()=>{if(!sel&&operator)setSel(operator)},[operator,sel]);
 ...
 if(!isAdmin)return <div className="stack"><div className="panel"><PanelTitle title="Meu usuário" subtitle="Perfil, processos e identidade da conta."/><div className="userIdentity"><div className="avatar largeAvatar">{initials(operator?.nome||"U")}</div><div><h2>{operator?.nome}</h2><p>{operator?.email}</p><span className="pill">Operador</span></div></div><div className="permissionGrid">{moduleList.map(([id,label])=><div className="permission" key={id}><span><b>{label}</b><small>{operator?.permissoes?.[id]===false?"Bloqueado":"Liberado"}</small></span></div>)}</div><div className="info">As permissões do usuário são administradas pelo proprietário.</div></div><div className="panel"><PanelTitle title="Canais" subtitle="Configurações do seu posto de atendimento."/><Omnichannel config={channelConfig} onSave={onSaveChannels}/></div><div className="panel"><PanelTitle title="Telefonia" subtitle="Configurações do seu posto."/><Telephony config={dialerConfig} onSave={onSaveDialer}/></div></div>;
 return <div className="stack"><div className="panel"><PanelTitle title="Meu usuário / Usuários" subtitle="Selecione qualquer usuário para configurar perfil, acesso e processos."/><div className="userGrid">{users.map(u=><button key={u.id} className={"userCard "+(sel?.id===u.id?"selected":"")} onClick={()=>setSel(u)}><div className="miniAvatar">{initials(u.nome)}</div><div><b>{u.nome}</b><small>{u.email}</small></div><span className={"pill "+(u.ativo?"":"warning")}>{u.ativo?"Ativo":"Inativo"}</span></button>)}</div></div><div className="panel"><div className="toolbar"><PanelTitle title={"Configurar: "+(sel?.nome||"Usuário")} subtitle={sel?.email||""}/><div className="actionRow"><select value={perfil} onChange={e=>setPerfil(e.target.value)}><option value="admin">Proprietário</option><option value="gestor">Gestor</option><option value="operador">Operador</option></select><label className="switchRow compact"><input type="checkbox" checked={ativo} onChange={e=>setAtivo(e.target.checked)}/><span>Ativo</span></label></div></div><div className="permissionGrid">{moduleList.map(([id,label])=><label className="permission" key={id}><input type="checkbox" checked={perm[id]!==false} onChange={e=>setPerm(x=>({...x,[id]:e.target.checked}))}/><span><b>{label}</b><small>{perm[id]===false?"Bloqueado":"Liberado"}</small></span></label>)}</div><button className="btn primary" onClick={()=>sel&&onSaveUser(sel.id,perm,sel.preferencias||{},ativo,perfil)}>Salvar permissões do usuário</button></div><div className="panel"><PanelTitle title="Configurações de canais" subtitle="WhatsApp, Instagram, Messenger, e-mail, SMS e IA."/><Omnichannel config={channelConfig} onSave={onSaveChannels}/></div><div className="panel"><PanelTitle title="Configurações de telefonia" subtitle="Discador, PABX, URA, filas, monitoria e gravações."/><Telephony config={dialerConfig} onSave={onSaveDialer}/></div><div className="panel"><PanelTitle title="Infraestrutura e segurança" subtitle="Resumo da arquitetura atual."/><div className="featureGrid"><Feature title="Segurança" state="Ativa" text="RLS, auditoria, cabeçalhos de segurança, limite de importação e separação de privilégios no banco."/><Feature title="LGPD operacional" state="Ativa" text="Não Perturbe centralizado, registro de ações e histórico de retornos."/><Feature title="Vercel + Supabase" state="Online" text="Frontend Next.js e PostgreSQL gerenciado, com dados privados protegidos por RLS."/><Feature title="Proprietário" state="Admin" text="Seu usuário tem controle dos processos e das permissões dos demais usuários."/></div></div></div>
}
