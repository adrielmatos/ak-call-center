...
function getEmbeddedConfig(){return{url:String(process.env.NEXT_PUBLIC_SUPABASE_URL||"").trim(),key:String(process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY||process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||"").trim()};}
export function isSupabaseConfigured(){const{url,key}=getEmbeddedConfig();return Boolean(url&&key);}
...
export async function ensureSupabaseConfig(){
 ...
 if(!configPromise){
  configPromise=fetch("/api/config",{method:"GET",cache:"no-store",credentials:"same-origin"}).then(async response=>{
...
export function createClient():SupabaseClient{
 ...
 browserClient=createBrowserClient(url,key);return browserClient;
}
export async function ensureClient():Promise<SupabaseClient>{
 if(browserClient)return browserClient;
 if(typeof window==="undefined")return createClient();
 const{url,key}=await ensureSupabaseConfig();browserClient=createBrowserClient(url,key);return browserClient;
}

const authProxy=new Proxy({} as SupabaseClient["auth"],{get(_target,property){
 if(property==="getSession")return()=>ensureClient().then(client=>client.auth.getSession()).catch(error=>({data:{session:null},error} as any));
...
export const supabase=new Proxy({} as SupabaseClient,{get(_target,property,receiver){
 ...
 const client=createClient();const value=Reflect.get(client as object,property,receiver);return typeof value==="function"?value.bind(client):value;
}});
